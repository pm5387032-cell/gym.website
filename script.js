...
bmicalc.addEventListener("click", () => {
  ...
  if (h > 0 && w > 0) {
    ...
    ansadvice.style.color="yellow"
  }
  else if(result <= 24.9){
        ansdefination.innerText ="Normal ✅";
        ansadvice.innerText = "aap fit ho aap roj excise kar sakte ho fit rahne ke liye "
        ansadvice.style.color ="green";

  }
  else if(result <= 29.9){
        ansdefination.innerText= "Overweight ⚠️";
        ansadvice.innerText = "tumhe bhajan lkam karne ki jarrurat hai "
         ansadvice.style.color= "orange";
  }
  else{
        ansdefination.innerText = "Obses ☠️";
        ansdefination.style.backgroundColor = "red"
        ansadvice.innerText = "aap ko sach me bhjan kam karne ki jarurat hai"
